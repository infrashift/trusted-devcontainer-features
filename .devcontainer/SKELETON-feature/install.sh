...
su -s /bin/bash ${DEVCONTAINER_FEATURE_USERNAME} <<EOF
    ${DEVCONTAINER_FEATURE_USER_LOCAL_BIN_PATH}/ansible-playbook --inventory ${DEVCONTAINER_FEATURE_USER_HOME}/hosts.yml activate-feature.yml -e "color=${COLOR_CHOICE}" -e "is_favorite_color=${IS_MY_FAVORITE_COLOR}"
EOF
echo ""

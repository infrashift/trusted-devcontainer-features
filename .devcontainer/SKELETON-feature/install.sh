...
su -s /bin/bash ${FEATURE_BOOTSTRAP_USERNAME} <<EOF
    ${FEATURE_BOOTSTRAP_USER_LOCAL_BIN_PATH}/ansible-playbook --inventory ${FEATURE_BOOTSTRAP_USER_HOME}/hosts.yml activate-feature.yml -e "color=${COLOR_CHOICE}" -e "is_favorite_color=${IS_MY_FAVORITE_COLOR}"
EOF
echo ""

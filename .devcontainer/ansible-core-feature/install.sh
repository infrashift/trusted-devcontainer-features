...
su -s /bin/bash ${SECUREDEVCONTAINER_FEATURE_USERNAME} <<EOF
    ${SECUREDEVCONTAINER_FEATURE_USER_LOCAL_BIN_PATH}/ansible-playbook --inventory ${SECUREDEVCONTAINER_FEATURE_USER_HOME}/hosts.yml activate-feature.yml -e "ansible_core_versions=${TARGET_VERSION}" -e "target_username=${SECUREDEVCONTAINER_USERNAME}"
EOF

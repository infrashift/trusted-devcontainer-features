...
su -s /bin/bash ${DEVCONTAINER_FEATURE_USERNAME} <<EOF
    ${DEVCONTAINER_FEATURE_USER_LOCAL_BIN_PATH}/ansible-playbook --inventory ${DEVCONTAINER_FEATURE_USER_HOME}/hosts.yml activate-feature.yml -e "cue_version=${TARGET_VERSION}" -e "target_username=${DEVCONTAINER_USERNAME}"
EOF
echo ""

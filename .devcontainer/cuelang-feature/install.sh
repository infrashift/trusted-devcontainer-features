...
echo "Activating feature 'CUE (CUElang)'...                                           "
echo "********************************************************************************"
echo "                                                                                "
...
# Find TARGET_CUELANGVERSION and TARGET_CUELANGCHECKSUM definitions in devcontainer-feature.json
echo "Activating DevContainer Feature..."
su -s /bin/bash ${SECUREDEVCONTAINER_FEATURE_USERNAME} <<EOF
    ${SECUREDEVCONTAINER_FEATURE_USER_LOCAL_BIN_PATH}/ansible-playbook --inventory ${SECUREDEVCONTAINER_FEATURE_USER_HOME}/hosts.yml activate-feature.yml -e "cue_version=${TARGET_VERSION}" -e "target_username=${SECUREDEVCONTAINER_USERNAME}"
EOF
echo ""

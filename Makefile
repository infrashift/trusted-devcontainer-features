...
.PHONY: check-published-drift
check-published-drift: ## Verify source matches what is published under its version (network)
	./scripts/check-published-drift.sh --check

.PHONY: check-fanout
check-fanout: ## Verify dependents are bumped alongside bootstrap (BASE=main)
	./scripts/check-bootstrap-fanout.sh $${BASE:-main}
...
.PHONY: check
check: check-contract check-version-pins check-fanout check-published-drift check-feature-refs check-workflows lint-shell check-policy repo-gate ## Run every static check

...
#
# Userland lane: both binaries are static and land under the user's home, so
# nothing here needs --privileged.
set -euo pipefail
...
# The checksum options are legitimately empty: empty means "use the pinned map".
# Only absence is a bug, so they use :- rather than :?.
exec /opt/bootstrap/run-feature.sh \
    --role ansible-role-feature \
    -e "_kaniko_version=${TARGET_VERSION}" \
    -e "_kaniko_executor_checksum=${TARGET_EXECUTOR_CHECKSUM:-}" \
    -e "_kaniko_warmer_checksum=${TARGET_WARMER_CHECKSUM:-}"

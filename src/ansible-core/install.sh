...
: "${TARGET_PYTHON_VERSION:?feature option 'target_python_version' resolved empty — devcontainer-feature.json must declare a default}"

exec /opt/bootstrap/run-feature.sh \
    --role ansible-role-feature \
    -e "_ansible_core_version=${TARGET_VERSION}" \
    -e "_ansible_core_python_version=${TARGET_PYTHON_VERSION}"

...
: "${TARGET_MAJOR_VERSION:?feature option 'target_major_version' resolved empty — devcontainer-feature.json must declare a default}"
...
exec /opt/bootstrap/run-feature.sh \
    ...
    -e "_openjdk_major_version=${TARGET_MAJOR_VERSION}" \
    -e "_openjdk_version=${TARGET_VERSION}" \
    -e "_openjdk_checksum=${TARGET_CHECKSUM:-}"
